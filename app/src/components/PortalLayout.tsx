...
export default function PortalLayout({ children, title, subtitle }: PortalLayoutProps) {
  ...
  return <div className="fixed inset-0 flex overflow-hidden" style={{ background: '#e8ebe6' }} dir={isRTL ? 'rtl' : 'ltr'}>
    {sidebarOpen && <div className="fixed inset-0 z-40 bg-black/50 lg:hidden" onClick={() => setSidebarOpen(false)} />}
    <aside className={cn('fixed inset-y-0 z-50 flex h-full w-[280px] flex-shrink-0 flex-col overflow-hidden bg-white transition-transform duration-300 lg:sticky lg:top-0 lg:block', isRTL ? 'border-l' : 'border-r', sidebarOpen ? 'translate-x-0' : isRTL ? 'translate-x-full lg:translate-x-0' : '-translate-x-full lg:translate-x-0')} style={{ borderColor: '#dfe1dd' }}>{sidebarContent}</aside>
