...
export default function CompanyAnalytics() {
  ...
  return (
    <PortalLayout title="Recruitment Analytics" titleAr="تحليلات التوظيف">
      {/* Header */}
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
        {metricList.map((m) => {
          const data = recruitmentMetrics[m.key as keyof typeof recruitmentMetrics]
          return (
            <MetricCard
              key={m.key}
              icon={m.icon}
              value={data.value}
              label={isRTL ? data.labelAr : data.label}
              trend={data.trend}
              iconBg={m.iconBg}
            />
          )
      ...
      <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        >
          <ResponsiveContainer width="100%" height={300}>
            <BarChart
              data={[
                { name: isRTL ? 'المتقدمين' : 'Applicants', count: 156 },
                { name: isRTL ? 'الفرز' : 'Screening', count: 89 },
                { name: isRTL ? 'المقابلات' : 'Interviews', count: 34 },
                { name: isRTL ? 'العروض' : 'Offers', count: 12 },
                { name: isRTL ? 'المعينين' : 'Hired', count: 8 },
              ]}
              layout="vertical"
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        >
          <ResponsiveContainer width="100%" height={300}>
            <BarChart
              data={[
                { range: '0-20%', count: 1, color: '#dc2626' },
                { range: '21-40%', count: 0, color: '#f59e0b' },
                { range: '41-60%', count: 2, color: '#f59e0b' },
                { range: '61-80%', count: 4, color: '#3b82f6' },
                { range: '81-100%', count: 8, color: '#9fe870' },
              ]}
              margin={{ top: 5, right: 30, left: 20, bottom: 5 }}
            >
              <CartesianGrid strokeDasharray="3 3" stroke="#dfe1dd" />
